...
use RedIRIS\PoA\PoAConstants;
use RedIRIS\PoA\PoAException;
use RedIRIS\PoA\PoAUtils;
 ...
 */
define('PAPI_DBA', 'RedIRIS\PoA\Conf\authn\PAPIDBADB');
define('PAPI_MYSQL', 'RedIRIS\PoA\Conf\authn\PAPIMySQLDB');
define('PAPI_SESSION', 'RedIRIS\PoA\Conf\authn\PAPISessionDB');
define('PAPI_MEMCACHED', 'RedIRIS\PoA\Conf\authn\PAPIMemcachedDB');
...
define('PROTO_ATTR_ASSERTION', '__assertion');
...
class PAPIAuthnEngine extends AuthenticationEngine {
    ...
    public function configure($file,$section) {
        ...
        // set default OPOA
        if (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != "off") $this->opoa .= "s";
    ...
    public function authenticate() {
        ...
        if ($action === "CHECKED" && !$auth) { // GPoA/AS response
            ...
            if (!$key) {
                $this->status = PoAConstants::AUTHN_FAILED;
                $this->dirty = false;
            ...
            if ($key) { // the request was validated
                if ($this->skip_redirection) {
                    $this->status = PoAConstants::AUTHN_SUCCESS;
                    return PoAConstants::AUTHN_SUCCESS;
                }                               
                // set a new cookie
                    ...
                    if (!$this->clean_url) {
                        ...
                        if ($_SERVER["REQUEST_METHOD"] === "POST") {
                            ...
                            foreach ($_POST as $name => $value) {
                                $inputs .= "<input name='".htmlentities(urldecode($name))."' type='hidden' value='".htmlentities(urldecode($value))."' />";
                            }
...
<?php
    ...
    public function getAttributes() {
        ...
        if (empty($this->attributes)) {
            ...
            foreach ($attrs as $attr) {
                $p = explode(NAMEVALUE_SEPARATOR, $attr);
                $name = $p[0];
                $value = $p[1];
    ...
    protected function getSingleLogoutLocation() {
        ...
        $sep = (strpos($this->cfg->getRedirectURL(), "?") !== false) ? "&" : "?";
    ...
    protected function getRedirectLocation() {
        ...
        if ($this->cfg->getRedirectType() === PoAConstants::AS_T) { // AS
    ...
    protected function saveRequest($key, $hli) {
        ...
        $ok = $this->db->replaceContents($key, $_GET, $_POST, $_REQUEST, $_SERVER["QUERY_STRING"],
                                         $_SERVER["REQUEST_METHOD"], $input, $hli);
        $this->db->close();
        return (!$ok) ? false : $key;
    }
    ...
    protected function loadRequest($key) {
        global $HTTP_RAW_POST_DATA, $_GET, $_POST, $_REQUEST, $_SERVER;
        ...
        $post_vars = explode("&", $request["PHP_INPUT"]);
        foreach ($post_vars as $parameter) {
        ...
        trigger_error("loadRequest QUERY_STRING ->".$request["QUERY_STRING"], E_USER_WARNING);
        trigger_error("loadRequest REQUEST_METHOD ->".$request["REQUEST_METHOD"], E_USER_WARNING);
        trigger_error("loadRequest PHP_INPUT ->".$request["PHP_INPUT"], E_USER_WARNING);
        return $request;
    }
    ...
    protected function getNewCookie() {
        ...
        $r = $this->crypto->encryptAES($content);
        return $r;
    }
