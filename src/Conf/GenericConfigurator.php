...
abstract class GenericConfigurator {
    ...
    public function __construct($file, $site) {
        ...
        if (is_readable($file)) {
            ...
            throw new PoAException('config-not-found', PoAConstants::CONFIG_ERR, array($file));
        }
        // check configuration
        $this->validate();
    }
    ...
    protected function configure($file, $site) {
        ...
        $name = strtolower(str_replace("Configurator", "", $subject[count($subject) - 1]))."_cfg";
        $set = eval("return isset(\$".$name.");");
        if (!$set) {
            throw new PoAException('config-err-php', PoAConstants::CONFIG_ERR, array($name, $site));
        }
